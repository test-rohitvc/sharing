...
# Import the custom tracer component
from ai_levate.tracing import LanfuseTracer
tracer = LanfuseTracer()
...
class KeycloakRBACManager:
    ...
    @tracer.observe(metadata={"name": "keycloak_create_user"})
    def create_user(self, username: str, email: str, password: str) -> str:
        tracer.update_metadata({"username": username, "email": email})
    ...
    @tracer.observe(metadata = {"name":"keycloak_get_users"})
    ...
    @tracer.observe(metadata = {"name":"keycloak_delete_user"})
    def delete_user(self, user_id: str):
        tracer.update_metadata({"user_id": user_id})
        self.admin.delete_user(user_id)

    @tracer.observe(metadata = {"name":"keycloak_login_user"})
    def login_user(self, username: str, password: str) -> dict:
        """Returns the JWT token for a human user."""
        tracer.update_metadata({"username": username})
        return self.openid.token(username, password)

    @tracer.observe(metadata = {"name":"keycloak_assign_user_role"})
    def assign_user_role(self, user_id: str, role_name: str):
        ...
        tracer.update_metadata({"user_id": user_id, "role_name": role_name})
        role = self.admin.get_realm_role(role_name)
        self.admin.assign_realm_roles(user_id=user_id, roles=[role])

    @tracer.observe(metadata = {"name":"keycloak_remove_user_role"})
    ...
    @tracer.observe(metadata = {"name":"keycloak_create_agent"})
    def create_agent(self, agent_name: str) -> str:
        """Creates the Agent Client AND auto-creates its matching 'invoke' role."""
        tracer.update_metadata({"agent_name": agent_name})
        client_id = (
            f"agent-{agent_name}" if not agent_name.startswith("agent-") else agent_name
        )
        payload = {
            "clientId": client_id,
    ...
    @tracer.observe(metadata = {"name":"keycloak_get_agents"})
    def get_agents(self) -> List[Dict]:
        clients = self.admin.get_clients()
        return [c for c in clients if c["clientId"].startswith("agent-")]

    @tracer.observe(metadata = {"name":"keycloak_delete_agent"})
    def delete_agent(self, client_uuid: str, client_id: str):
        tracer.update_metadata({"client_uuid": client_uuid, "client_id": client_id})
    ...
    @tracer.observe(metadata = {"name":"keycloak_assign_agent_tool"})
    def assign_agent_tool(self, client_uuid: str, tool_name: str):
        """Grants a tool (role) to an agent's service account."""
        tracer.update_metadata({"client_uuid": client_uuid, "tool_name": tool_name})
        formatted_tool = (
            f"tool:{tool_name}" if not tool_name.startswith("tool:") else tool_name
        )
    ...
    @tracer.observe(metadata = {"name":"keycloak_remove_agent_tool"})
    ...
    @tracer.observe(metadata = {"name":"keycloak_get_user_role_matrix"})
    def get_user_role_matrix(self) -> Dict[str, List[str]]:
        ...
        for user in users:
            try:
                ...
                invoke_roles = [
                    r["name"].replace("invoke:", "")
                    for r in roles
                    if r["name"].startswith("invoke:")
                ]
    ...
    @tracer.observe(metadata = {"name":"keycloak_get_agent_role_matrix"})
    ...
    @tracer.observe(metadata = {"name":"keycloak_get_active_sessions"})
    def get_active_sessions(self, target_client_id: Optional[str] = None) -> List[Dict]:
        ...
        tracer.update_metadata({"target_client_id": client_to_check})
        ...
        try:
            ...
            sessions = self.admin.get_client_all_sessions(client_uuid)
            ...
            tracer.update_metadata({"active_sessions_count": len(cleaned_sessions)})
